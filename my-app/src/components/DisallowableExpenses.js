import {AddpropertyDisAllowableExpense,GetMonthlyDisallowableExpenses} from '../ApiServices/AddExpenses';
...

{
    ...
    const handleSubmit = async (e) => {
        ...
        const DisAllowableExpenseDate = reverseDate(e.target.elements.dateIncurred.value)
        const id = params.id;
        ...
        try {
            const responseExpenses = await AddpropertyDisAllowableExpense(DisAllowableExpense)
                        
            if(responseExpenses.status === 201){
                SetDisallowableExpenses((prev)=>[...prev,responseExpenses.data] )
            } else {
    ...
    return (
        <div>
            ...
            {showDisallowableExpenses.map((expense) => {
            ...
            <button onClick={() => setShowForm(!showForm)}>Add Disallowable Expense</button>
            {showForm && (
